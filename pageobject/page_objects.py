...
class PageElement(object):
    ...
    def get_element(self, context):
        try:
            ...
            log.error('未找到操作元素,method:{}; value:{}'.format(self.locator[0], self.locator[1]))
            raise NoSuchElementException
        else:
            try:
                style_red = 'arguments[0].style.border="2px solid red"'
                context.execute_script(style_red, elem)
                log.info('定位操作元素,method:{}; value:{}'.format(self.locator[0], self.locator[1]))
    ...
    def find(self, context):
        for i in range(1, self.time_out):
            ...
            get_elem = self.get_element(context)
    ...
    def __set__(self, instance, value):
        ...
        log.info('输入:{value},method:{method}; value:{selector}'.format(value=value, method=self.locator[0],
